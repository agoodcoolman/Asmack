...
public class PingManager {
    ...
    private Set<PingSucessListener> pingSucessListeners = Collections
            .synchronizedSet(new HashSet<PingSucessListener>());
    ...
    public void registerPingSucessListener(PingSucessListener listener) {
    	pingSucessListeners.add(listener);
    }
    
    public void unregisterPingSucessListener(PingSucessListener listener) {
    	pingSucessListeners.remove(listener);
    }
    ...
    /**
     * @param pingIntervalTime  延时心跳,时间按照秒算的.
     */
    public void maybeScedulePingServerTask(int pingIntervalTimeSECONDS) {
    	maybeStopPingServerTask();
    	// 按照重新设置的时间进行心跳.
    	
    	setPingIntervall(pingIntervalTimeSECONDS);
        if (pingInterval > 0) {
            periodicPingTask = periodicPingExecutorService.schedule(new ServerPingTask(connection), pingIntervalTimeSECONDS,
                    TimeUnit.SECONDS);
        }
    }
